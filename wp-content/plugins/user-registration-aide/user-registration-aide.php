	...
	*/
	...
	function CSDS_USER_REG_AIDE() { //constructor
		...
		if( empty($options['updated'] ) ){
			...
			$ura_options->csds_userRegAide_updateOptions();
		}elseif( $options['csds_userRegAide_db_Version'] != '1.5.1.0' ){
			$ura_options->csds_userRegAide_updateOptions();
		}
		// defines
		...
		if( class_exists( $class ) && is_plugin_active( $plugin ) ){
			...
			add_filter( 'registration_errors', array( &$regForm, 'ura_show_user_pending_message' ), 20, 3 );
			...
			add_action('login_message', array( &$actions, 'ura_register_message' ) );
			unset( $pw_nua );
		}
		...
		if( isset( $_GET['action'] ) && $_GET['action'] == 'register' ){
			...
			add_action( 'known_fields_rf', array( &$regForm, 'display_known_fields' ), 1, 3 );
			add_action( 'tml_known_fields_rf', array( &$regForm, 'display_known_fields_theme_my_login' ), 1, 3 );
		...
		add_action( 'start_msg_wrapper', array( &$actions, 'start_wp_msg_wrapper' ), 10, 6 );
		add_action( 'create_msg_tabs', array( &$actions, 'msg_options_tabs_page' ), 10, 2 );
		...
		//add_action( 'custom_options_update', array( &$uco, 'custom_options_views' ) );
		//add_action( 'custom_options_view', array( &$uco, 'custom_options_msg_views' ), 10, 4 );
...
if( !function_exists( 'wp_new_user_notification' ) ){
	function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		...
		if( !class_exists( $class ) && is_plugin_active( $plugin ) ){
			...
			//$message  = sprintf( __( 'New user registration on your site ', 'csds_userRegAide' ) );
			//$message .= $blogname . "\r\n\r\n";
			$message .= sprintf( __( 'Username: %s', 'csds_userRegAide' ), $user->user_login . "\r\n\r\n" );
			//$message .= sprintf( __( 'Username: ', 'csds_userRegAide' ) );
			//$message .= $user->user_login . "\r\n\r\n";
			$message .= sprintf( __( 'E-mail: %s', 'csds_userRegAide' ), $user->user_email . "\r\n\r\n" );
			//$message .= sprintf( __( 'E-mail: ', 'csds_userRegAide' ) );
		...
		//$message = sprintf( __( 'Thank you for registering with ', 'csds_userRegAide' ) );
		//$message .=  $blogname. "\r\n\n";
		...
		//$message .= sprintf( __( 'Here are your new login credentials for ', 'csds_userRegAide' ) );
		//$message .=  $blogname. "\r\n\n";
		...
		if( class_exists( $class ) && is_plugin_active( $plugin ) ){
			...
			if( in_array( 'Password', $fields ) ){
				...
				//$message .= __( 'To set your password, visit the following address:', 'csds_userRegAide'  ) . "\r\n\r\n";
