	...
	function csds_userRegAide_regFormOptions(){
		...
		if( isset( $_POST['reg_form_message_update'] ) ){
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regForm'], 'csds-regForm' ) ){
				...
				if( $ucnt == 0 ){
					...
					$msg = '<div class="updated"><p>'. __('New Bottom Registration Form Message Options updated successfully.', 'csds_userRegAide') .'</p></div>'; //Report to the user that the data has been updated successfully
				}else{
					$msg = '<div class="updated"><p>'. __( 'New Bottom Registration Form Message Options empty, not updated successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		...
		// handles password strength requirement options
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regForm'], 'csds-regForm' ) ){
				...
				if( !empty( $_POST['csds_select_DefaultPSR'] ) && !empty( $_POST['csds_select_CustomPSR'] ) ){ 		  //	Updates default password strength requirements
					//$update['default_xwrd_strength'] = sanitize_text_field( $_POST['csds_select_DefaultPSR'] );
				...
				if( $ucnt == 0 ){
					...
					$msg = '<div class="updated"><p>'. __( 'New Password Strength Requirement Options empty, not updated successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		// add code to handle new registration & login form redirects
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regForm'], 'csds-regForm' ) ){
				...
				update_option( 'csds_userRegAide_Options', $update );
				if( $ucnt == 0 ){
					$msg = '<div class="updated"><p>'. __( 'New Login & Registration Form Custom Redirects Options updated successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}else{
					$msg = '<div class="updated"><p>'. sprintf( __( 'New Login & Registration Form Custom Redirects Options Field %s Empty!', 'csds_userRegAide' ), $field ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		...
		// Updates registration form agreement message options
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regForm'], 'csds-regForm' ) ){
				...
				update_option( 'csds_userRegAide_Options', $update );
				if( $ucnt == 0 ){
					$msg = '<div class="updated"><p>'. __( 'New Registration Form Agreement Message Options updated successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}else{
					$msg = '<div class="updated"><p>'. sprintf( __( 'New Registration Form Agreement Message Options Fields %s Empty!', 'csds_userRegAide' ), $field ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		...
		// Upates anti-spam math problem settings 
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regForm'], 'csds-regForm' ) ){
				...
				if( empty( $errors ) || $errors == '' ){
					$msg = '<div class="updated"><p>'. __( 'Anti-Bot-Spammer Math Problem Options updated successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
					update_option("csds_userRegAide_Options", $update);
				}else{
					$msg = '<div class="updated"><p>'. __( "You have not selected any operators (+. -, /, *) to use and selected to use the anti-spam math problem! Please try again and select at least one operator of select no to use the anti-spam math problem", 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
		...
		// Upates and changes profile page extra fields title 
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regForm'], 'csds-regForm' ) ){
				...
				if( $ucnt == 0 ){
					$msg = '<div class="updated"><p>'. __( 'New Profile Page Extra Fields Title Updated Successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
					update_option("csds_userRegAide_Options", $update);
				}else{
					$msg = '<div class="updated"><p>'. sprintf( __( 'New Profile Page Extra Fields Title Fields %s Empty!', 'csds_userRegAide' ), $field ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		...
		if(current_user_can('manage_options', $current_user->ID)){	
			...
			$span = array( 'regForm', __( 'Add Your Own Message to Bottom of Registration Form:', 'csds_userRegAide' ), 'csds_userRegAide' );
			...
			$span = array( 'regForm',  __( 'Add custom redirects after users login or a new user registers here:', 'csds_userRegAide' ), 'csds_userRegAide' );
			do_action( 'start_mini_wrap', $span ); ?>	
				<table class="regForm" width="100%">
					<tr>
			...
			$span = array( 'regForm', __( 'Add Your Own Agreement Message and Policy Link with Confirmation of Agreement to Bottom of Registration Form:', 'csds_userRegAide' ), 'csds_userRegAide' );
			do_action( 'start_mini_wrap', $span ); ?>
			<table class="regForm" width="100%">
				<tr>
			...
			$span = array( 'regForm', __( 'Add Anti-Bot-Spammer for Registration Form:', 'csds_userRegAide' ), 'csds_userRegAide' );
			do_action( 'start_mini_wrap', $span ); ?>
				<table class="regForm" width="100%">
					<tr>
					<td>
