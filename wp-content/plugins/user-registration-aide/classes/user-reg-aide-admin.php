...
class CSDS_URA_ADMIN_SETTINGS
{
	...
	*/
	...
	function csds_userRegAide_myOptionsSubpanel(){
		...
		if( $options['csds_userRegAide_db_Version'] != "1.5.1.0" ){
		...
		if( isset( $_POST['dash_widget_display_option'] ) ){
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFieldsAdmin'], 'csds-regFieldsAdmin' ) ){	
				...
				$msg = $dashWidget->update_dashboard_widget_options();
				//exit ( ' - - - - - - - - -  '.$msg.' - - - - - - - - - - ' );
			}
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFieldsAdmin'], 'csds-regFieldsAdmin' ) ){	
				...
				$msg = $dashWidget->update_dashboard_widget_fields();
				//exit ( ' - - - - - - - - -  '.$msg.' - - - - - - - - - - ' );
			}
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFieldsAdmin'], 'csds-regFieldsAdmin' ) ){				
				...
				$msg = $dashWidget->update_dashboard_widget_field_order();
				//exit ( ' - - - - - - - - -  '.$msg.' - - - - - - - - - - ' );
			}
			...
			$msg = '<div class="updated"><p>'. __( 'Registration Form Field Options Updated Successfully!', 'csds_userRegAide' ) .'</p></div>';
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFieldsAdmin'], 'csds-regFieldsAdmin' ) ){	
				...
				if( current_user_can( 'activate_plugins', $current_user->ID ) ){
					...
					$msg = '<div class="updated"><p>'. __( 'Registration Form Add New Field Options Updated Successfully!', 'csds_userRegAide' ) .'</p></div>';					//Report to the user that the data has been updated successfully
				}else{
					$ext = '<div class="error"><p>'. __( 'You do not have adequate permissions to edit this plugin! Please check with Administrator to get additional permissions.', 'csds_userRegAide' ) .'</p></div>';
					exit( $ext );
				}
			}
			...
			$optional_fields = array();
			update_option( 'csds_ura_optionalFields', $optional_fields );
			$msg = '<div class="updated"><p>'. __( 'Registration Form Optional Field Options Updated Successfully!', 'csds_userRegAide' ) .'</p></div>';
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFieldsAdmin'], 'csds-regFieldsAdmin' ) ){	
				...
				if( current_user_can( 'activate_plugins', $current_user->ID ) ){
							...
							if( !empty( $results2 ) ){
								foreach( $results2 as $key => $value ){
									if( !empty( $registrationFields ) ){
										...
										update_option( "csds_ura_optionalFields", $optional_fields );
										$msg = __( 'Registration Form Optional Fields Successfully Updated!', 'csds_userRegAide' );
										$class = 'updated';
									}else{
										$msg = __( 'No Registration Form Fields Exist, Select Some Registration Form Fields First!', 'csds_userRegAide' );
										$class = 'error';
									}
					...
					$msg = '<div class="'.$class.'"><p>'. __( $msg, 'csds_userRegAide' ) .'</p></div>';					//Report to the user that the data has been updated successfully
				}else{
					$ext = '<div class="error"><p>'. __( 'You do not have adequate permissions to edit this plugin! Please check with Administrator to get additional permissions.', 'csds_userRegAide' ) .'</p></div>';
					exit( $ext );
				}
		...
		// handles required fields asterisk update
			...
			update_option( "csds_userRegAide_Options", $options );
			$msg = '<div class="updated"><p>'. __( 'Asterisk Options Updated Successfully!', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
		// Handles adding new fields to database for user profiles and registration
			...
			// Checking for blank fields
				if( $_POST['csds_userRegAide_newFieldKey'] == '' && $_POST['csds_userRegAide_newField'] == ''){
					$seperator = " & ";
					$csds_userRegMod_fields_missing = __( ' New Field Key & New Field Name ', 'csds_userRegAide' );
					$number_fields = 2;
					
				}elseif( $_POST['csds_userRegAide_newFieldKey'] == '' ) {
					$csds_userRegMod_fields_missing1 = __( ' New Field Key ', 'csds_userRegAide' );
					$csds_userRegAide_newField = sanitize_text_field( trim( $_POST['csds_userRegAide_newField'] ) );
					$csds_userRegMod_fields_missing = $csds_userRegMod_fields_missing1;
					$field_missing = sanitize_text_field( trim( $_POST['csds_userRegAide_newField'] ) );
					$duplicate = $this->duplicate_fields( $field_missing );
					$number_fields = 1;
				}elseif( $_POST['csds_userRegAide_newField'] == '' ){
					$csds_userRegMod_fields_missing2 = __( ' New Field Name ', 'csds_userRegAide' );
					$csds_userRegAide_newFieldKey = sanitize_key( $_POST['csds_userRegAide_newFieldKey'] );
					$csds_userRegMod_fields_missing = $csds_userRegMod_fields_missing2;
					$field_missing = sanitize_key( $_POST['csds_userRegAide_newFieldKey'] );
					$duplicate = $this->duplicate_fields( $field_missing );
					$number_fields = 1;
				}else{
				...
				if( $csds_userRegMod_fields_missing != '' && $number_fields == 2 && $duplicate == 0 ){
					...
					$results = sanitize_text_field( $_POST['csds_userRegAide_newFieldKey'] );
					$results = $this->replace_key_spaces( $results );
					$results = sanitize_key( $results );
					$new_field = sanitize_text_field( $_POST['csds_userRegAide_newField'] );
					...
					if( !empty( $newFields ) ){
						foreach( $newFields as $key => $field ){
							if( $key != $results && $field != $new_field ){
								$msg = '<div class="updated"><p>'. __( 'Add New Field ', 'csds_userRegAide' );
								$msg .= ' '.$new_field;
								$msg .= __( ' option updated successfully!', 'csds_userRegAide').' </p></div>';
							}elseif( $key == $results ){
								$msg = '<div class="error"><p>'. sprintf( __( '***Cannot add duplicate fields, %s is already included in the extra fields!***', 'csds_userRegAide' ), $results ) .'</p></div>';
								break;
								
							}elseif( $field == $new_field ){
								$msg = '<div class="error"><p>'. __( '***Cannot add duplicate fields, ', 'csds_userRegAide' );
								$msg .=  ' '.$new_field;
								$msg .= __( ' is already included in the extra fields!***', 'csds_userRegAide') .'</p></div>';
								break;
							}else{
							 // do nothing yet
							}
		...
		if(current_user_can('manage_options', $current_user->ID)){	
			...
			$h2 = array( 'adminPage', __( 'User Registration Aide: Force New User Registration Fields Administration Page', 'csds_userRegAide' ), 'csds_userRegAide' );
			$span = array( 'regForm', __( 'Choose to display Creative Software Design Solutions Dashboard Widget Here:', 'csds_userRegAide' ), 'csds_userRegAide' );
			...
			?>
				...
				$span = array( 'regForm', __( 'Add Existing Fields To Registration Form or Create New Fields Here:', 'csds_userRegAide' ), 'csds_userRegAide' );
				...
				<?php
					...
					<tr>
						...
						if( !empty( $registratrionFields ) ){
							if( is_array( $regFields ) ){
								if( !empty( $knownFields ) && is_array( $knownFields ) ){
									foreach( $knownFields as $key1 => $value1 ){
										if( in_array( $value1, $regFields ) ){
											if( $key1 != 'user_pass' ){
								...
								if( !empty( $newFields ) && is_array( $newFields ) ){
									foreach( $newFields as $key2 => $value2 ){
										if( in_array( $value2, $regFields ) ){
											if( !empty( $optional_fields ) ){
												if( in_array( $value2, $optional_fields ) ){
													$selected = "selected=\"selected\"";
												}else{
													$selected = NULL;
												}
							...
							echo '<option value="no_registration_fields" >'. __( 'Hold down "Ctrl" button on keyboard to select or unselect multiple options!', 'csds_userRegAide' ) .'</option>';
						}
	...
	/* Checks new field key for blank spaces and replaces them
	* @since version 1.5.1.0
	* @param string $key
	* @return $key
	* @access public
	* @author Brian Novotny
	...
	*/
	...
	function replace_key_spaces( $key ){
		$new_key = ( string ) '';
