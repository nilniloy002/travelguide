 ...
 *
 * @category Class URA_REGISTRATION_FORM
 * @since 1.3.0
 * @updated 1.5.1.0
 * @access public
 * @author Brian Novotny
...
class URA_REGISTRATION_FORM
{
	...
	function URA_REGISTRATION_FORM() { //constructor
		...
		global $wp_version, $new_fields, $numb1, $numb2, $op, $admin, $admin_page, $ual, $ual_admin_settings; 
		self::$instance = $this;
	...
	*/
	...
	function display_password_fields(){
		...
		<p class="user-pass1-wrap">
			<label for="pass1"><?php _e( 'Password', 'csds_userRegAide' ); ?></label><br />
			<div class="wp-pwd">
	...
	/**
	 * Add other input fields to the new user registration page with theme my login plugin inactive
	 * @since 1.5.1.0
	 * @updated 1.5.1.0
	 * @access public
	 * @accepts string $fieldKey -- key for field used as name & id
	 * @accepts string $fieldName -- used for field name visible to user
	 * @accepts string $value -- input value for field if any
	 * @returns nothing
	 * @author Brian Novotny
	 * @website http://creative-software-design-solutions.com
	*/
	...
	/**
	 * Add other input fields to the new user registration page with theme my login plugin active
	 * @since 1.5.1.0
	 * @updated 1.5.1.0
	 * @access public
	 * @accepts string $fieldKey -- key for field used as name & id
	...
	*/
	...
	function display_known_fields( $fieldKey, $fieldName, $value ){
		...
		if( $fieldKey == 'first_name' ){
			?>
			<label><?php _e( 'First Name', 'csds_userRegAide' ); ?><?php echo $label; ?><br />
			<?php
		}elseif( $fieldKey == 'last_name' ){
			?>
			<label><?php _e( 'Last Name', 'csds_userRegAide' ); ?><?php echo $label; ?><br />
			<?php
		}elseif( $fieldKey == 'nickname' ){
			?>
			<label><?php _e( 'Nickname', 'csds_userRegAide' ); ?><?php echo $label; ?><br />
			<?php
		}elseif( $fieldKey == 'user_url' ){
			?>
			<label><?php _e( 'Website', 'csds_userRegAide' ); ?><?php echo $label; ?><br />
			<?php	
		}else{
			?>
			<label><?php _e( $fieldName, 'csds_userRegAide' ); ?><?php echo $label; ?><br />
			<?php
		}
		?>
		<input type="text" name="<?php echo $fieldKey; ?>" id="<?php echo $fieldKey; ?>" autocomplete="on" class="input" value="<?php echo $value;?>" size="25" style="font-size: 20px; width: 97%;	padding: 3px; margin-right: 6px;" /></label>
							...
							if( $id == $fieldKey ){
								if( !is_plugin_active('theme-my-login/theme-my-login.php' ) ){ // Compensates for theme my login bug
									if( $fieldKey == 'user_pass' ){ // adding password fields to form
										...
										if( array_key_exists( $fieldKey, $knownFields ) ){
				...
				foreach( $regFields as $thisValue => $fieldName ){
					if( $_POST[$thisValue] != '' ){
						if( $thisValue == "first_name" ){
							...
							$newWebsite = sanitize_text_field( $newWebsite );
							$newWebsite = esc_url_raw( $newWebsite );
				...
				if( $thisValue != "user_pass" ){
					if( !empty( $optional_fields ) && is_array( $optional_fields ) ){
						if( !array_key_exists( $thisValue, $optional_fields ) ){
							if( $_POST[$thisValue] == '' ) {
								$err_msg = __( '<strong>ERROR</strong>: Please type your ', 'csds_userRegAide');
								$err_msg .= $csds_userRegAide_registrationFields[$thisValue];
								$errors->add( 'empty_'.$thisValue , $err_msg );
								$error ++;
									...
									if( $thisValue1 != "user_pass" ){
										if( $thisValue != $thisValue1 ){
											if( $_POST[$thisValue] == $_POST[$thisValue1] ){
												$err_msg = __( '<strong>ERROR</strong>: Your ','csds_userRegAide');
												$err_msg .= $csds_userRegAide_registrationFields[$thisValue];
												$err_msg .= __( ' and ','csds_userRegAide');
												$err_msg .= $csds_userRegAide_registrationFields[$thisValue1];
												$err_msg .= __( ' are the same, please enter different values! ','csds_userRegAide');
												$errors->add( 'duplicate_spam_check'.$thisValue , $err_msg );
											}
										}
									}
								}
					...
					*/
					...
					if( strlen( trim( $_POST['pass1'] ) ) < $options['xwrd_length'] ){ // password length too short
						if( $options['default_xwrd_strength'] == 1 || ( $options['custom_xwrd_strength'] == 1 && $options['require_xwrd_length'] == 1 ) ){
							$errors->add( 'password_too_short', __( "<strong>ERROR</strong>: Password length too short! Should be at least 8 characters long!", 'csds_userRegAide' ) );
								$error ++;
						}
