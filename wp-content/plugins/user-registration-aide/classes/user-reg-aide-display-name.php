...
class URA_DISPLAY_NAME
{
	...
	*/
	...
	function user_display_name_view(){
		...
		$options = get_option('csds_userRegAide_Options');
		$span = array( 'regForm', __( 'Choose User Display Name Options Here:', 'csds_userRegAide' ), 'csds_userRegAide' );
		...
		if( !current_user_can( 'manage_options', $current_user->ID ) ){	
			...
			<table class="displayName">
				...
				<tr>
					<td>
					<p><label for="csds_profileDispNameYN" title="<?php _e( 'Using this option will remove the option for users to choose their own display name on the user profile and will give total control of user display names to the administrators of this site', 'csds_userRegAide' );?>"><?php _e('Allow User To Update Own Display Name in User Profile: ', 'csds_userRegAide');?><input type="radio" id="csds_profileDispNameYN" name="csds_profileDispNameYN"  value="1" <?php
