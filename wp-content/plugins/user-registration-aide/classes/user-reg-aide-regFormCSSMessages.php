	...
	function csds_userRegAide_regFormCSSOptions(){
		...
		if( isset( $_POST['reg_form_login_message_update'] ) ){
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFormCSSMsgs'], 'csds-regFormCSSMsgs' ) ){
				...
				if( $ecnt == 0 ){
					...
					$msg = '<div class="error"><p>'. __( 'New Top Registration Form Message Options Fields Empty ', 'csds_userRegAide' ) . $efield .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		...
		//updates login/registration form custom css options
			...
			if( wp_verify_nonce( $_POST['wp_nonce_csds-regFormCSSMsgs'], 'csds-regFormCSSMsgs' ) ){
				...
				if( !empty( $_POST['csds_userRegAide_show_shadow'] ) ){
					$update['show_shadow'] = sanitize_text_field( $_POST['csds_userRegAide_show_shadow'] );
						if( !empty($_POST['csds_userRegAide_shadowSize']) && $_POST['csds_userRegAide_show_shadow'] == 1 ){
							$update['shadow_size'] = sanitize_text_field( $_POST['csds_userRegAide_shadowSize'] );
				...
				update_option("csds_userRegAide_Options", $update);
				if( $ecnt == 0 ){
					$msg = '<div class="updated"><p>'. __( 'New Registration Form Logo Options updated successfully.', 'csds_userRegAide' ) .'</p></div>'; //Report to the user that the data has been updated successfully
				}else{
					foreach( $efields as $key => $value ){
						if( $key == 1 ){
							$efield = $value;
					...
					$msg = '<div class="error"><p>'. __( 'New Registration Form Logo Options Empty ', 'csds_userRegAide' ) . $efield .'</p></div>'; //Report to the user that the data has been updated successfully
				}
			}
		...
		if( current_user_can( 'manage_options', $current_user->ID ) ){
			...
			$span = array( 'regForm', __( 'Add Your Own Custom Message to Top of Login & Registration Forms:', 'csds_userRegAide' ), 'csds_userRegAide' );
			...
			<table class="regForm" width="100%">
				...
				<tr>
					...
					<input  style="width: 100px;" type="text" title="<?php _e( 'Enter the new size of shadow for login/registration page links in PX for your site! Example: 2px', 'csds_userRegAide' );?>" value="<?php _e( esc_attr( $options['shadow_size'] ), 'csds_userRegAide' );?>" name="csds_userRegAide_shadowSize" id="csds_userRegAide_newTextColor" />
					</td>
