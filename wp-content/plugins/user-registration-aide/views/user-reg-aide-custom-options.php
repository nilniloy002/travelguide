...
class URA_CUSTOM_OPTIONS
{
	...
	function custom_options_views(){
		...
		if( current_user_can( 'manage_options', $current_user->ID ) ){	
			//do_action( 'custom_options_view', $msg );
			
			$tab = 'custom_options';
			$h2 = array( 'adminPage', __( 'User Registration Aide: Custom Options', 'csds_userRegAide' ), 'csds_userRegAide' );
			$span = array( 'regForm', __( 'Change Custom Settings Here:', 'csds_userRegAide' ), 'csds_userRegAide' );
	...
	function custom_options_msg_views( $msg ){
		global $current_user;
